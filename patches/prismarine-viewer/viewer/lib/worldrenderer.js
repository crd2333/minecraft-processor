...
class WorldRenderer {
  constructor (scene, numWorkers = 4) {
    ...
    for (let i = 0; i < numWorkers; i++) {
      ...
      let src = __dirname
      if (typeof window !== 'undefined') src = 'vendor/packages/prismarine-viewer/public/worker.js'
      else src += '/worker.js'
  ...
  updateTexturesData () {
    loadTexture(this.texturesDataUrl || `vendor/packages/prismarine-viewer/public/textures/${this.version}.png`, texture => {
    ...
    const loadBlockStates = () => {
      return new Promise(resolve => {
        ...
        return loadJSON(`vendor/packages/prismarine-viewer/public/blocksStates/${this.version}.json`, resolve)
      })
    }
