...
function buildPixal3DMetadata (width, height, options) {
  const opts = options || {}
  const sourceContext = (pixal3dExportContext && pixal3dExportContext.source) || {}
  ...
  const fovY = THREE.MathUtils.degToRad(viewer.camera.fov)
  ...
  return {
    ...
    asset: pixal3dExportContext ? pixal3dExportContext.asset : null,
    source: {
      coordinate_space: 'minecraft_unified_blocks',
      block_point: 'center',
...
window.__captureScreenshot = function (options) {
  ...
  const captured = withTemporaryCameraAspect(viewer.camera, resolved.w / resolved.h, function () {
    return {
      ...
      pixal3d: buildPixal3DMetadata(resolved.w, resolved.h, options)
    }
  })
  return {
...
window.__captureGBuffer = function (options) {
  ...
  const captured = withTemporaryCameraAspect(viewer.camera, w / h, function () {
    ...
    const pixal3d = buildPixal3DMetadata(w, h, opts)
