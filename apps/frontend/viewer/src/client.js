...
const { buildCameraMetadata, buildPixal3DMetadata } = require('../../shared/pixal3d-metadata')
...
window._pw_controls = controls
window._pw_socket = socket

let firstPositionUpdate = true
let pixal3dExportContext = null
...
socket.on('pixal3dExportContext', (payload) => {
  pixal3dExportContext = payload || null
})
...
window.__captureScreenshot = function (options) {
  ...
  const wr = viewer.world
  const captured = withTemporaryCameraAspect(viewer.camera, resolved.w / resolved.h, function () {
    return {
      pixels: wr.renderRgbMap(renderer, viewer.camera, resolved.w, resolved.h),
      camera: buildCameraMetadata(viewer.camera, resolved.w, resolved.h),
      pixal3d: buildPixal3DMetadata(THREE, viewer.camera, pixal3dExportContext, resolved.w, resolved.h, options)
    }
  })
  return {
    dataUrl: pixelsToDataUrl(captured.pixels, resolved.w, resolved.h),
    width: resolved.w,
    height: resolved.h,
...
window.__captureGBuffer = function (options) {
  ...
  const captured = withTemporaryCameraAspect(viewer.camera, w / h, function () {
    ...
    const pixal3d = buildPixal3DMetadata(THREE, viewer.camera, pixal3dExportContext, w, h, opts)
    ...
    return { rgbPixelsRaw, depthPixels, maskPixelsRaw, segPixels, pixal3d }
  })
  ...
  const metadata = {
    ...
    camera: {
      ...
      view: buildCameraMetadata(viewer.camera, w, h)
    },
    pixal3d: captured.pixal3d,
